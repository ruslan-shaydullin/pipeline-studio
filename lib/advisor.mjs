...
export function reviewSchemaFor(pipeline) {
  const schema = structuredClone(reviewSchema);
  const ids = pipeline.stages.map((s) => s.id);
  if (ids.length)
    schema.properties.findings.items.properties.stageIds.items.enum = ids;
  else schema.properties.findings.items.properties.stageIds.maxItems = 0;
  schema.properties.proposal.properties.stages.items.properties.sourceId.enum =
    [...ids, null];
  return schema;
}
...
export function validateReviewResult(value, pipeline) {
  ...
  for (const f of value.findings)
    if (
      ...
      f.stageIds.some((id) => typeof id !== 'string')
    )
  ...
  return {
    ...value,
    // Finding links are advisory metadata. An unknown link must not discard
    // the analysis; stage identity in the applicable proposal stays strict.
    findings: value.findings.map((f) => ({
      ...f,
      stageIds: f.stageIds.filter((id) => ids.has(id)),
    })),
  };
}
